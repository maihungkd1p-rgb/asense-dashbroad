...
st.set_page_config(
    page_title="Báo Cáo Tự Động - a.SENSE", page_icon="📊", layout="wide"
)
...
def load_data():
    ...
    df_june["Ngày"] = pd.to_datetime(
        df_june["Ngày"], format="%Y-%m-%d", errors="coerce"
    ).dt.normalize()
    df_june["Doanh thu"] = pd.to_numeric(df_june["Doanh thu"], errors="coerce").fillna(
        0
    )
    ...
    df_july["Doanh thu"] = pd.to_numeric(df_july["Doanh thu"], errors="coerce").fillna(
        0
    )
    df_july["Ngày"] = (
        pd.to_datetime(df_july["Ngày"], errors="coerce", utc=True)
    ...
    # Chỉ lọc theo cửa hàng, việc lọc ngày sẽ do bộ lọc UI đảm nhiệm
    df_june = df_june[df_june["Mã cửa hàng"].isin(["L1 Landmark", "Solforest"])].copy()
    df_july = df_july[df_july["Mã cửa hàng"].isin(["L1 Landmark", "Solforest"])].copy()

    # Category extraction
    def get_category(row):
        if str(row["Nhóm dịch vụ"]).upper() == "RETAIL":
            return "Retail"
        note = str(row["Ghi chú"]).lower()
        if pd.isna(row["Ghi chú"]) or note == "nan":
            return "Khác"
        if any(
            k in note
            for k in ["nail", "nhặt da", "sơn gel", "chà gót", "tạo cầu", "phá"]
        ):
            return "Nails & Hand Care"
        if any(k in note for k in ["triệt", "mép", "nách"]):
            return "Triệt lông"
        if any(k in note for k in ["detox"]):
            return "Detox cổ vai gáy"
        if any(
            k in note
            for k in ["mặt nạ", "tăng sáng", "da tươi", "vtm", "mna", "mạt nạ"]
        ):
...
with st.expander("🎛️ BỘ LỌC DỮ LIỆU", expanded=True):
    ...
    with f_col1:
        selected_stores = st.multiselect(
            "🏪 Cơ Sở",
            df_all["Mã cửa hàng"].unique(),
            default=df_all["Mã cửa hàng"].unique(),
        )
    with f_col2:
        selected_categories = st.multiselect(
            "📂 Danh Mục",
            df_all["Danh mục"].unique(),
            default=df_all["Danh mục"].unique(),
        )
    with f_col3:
        selected_nhom_dv = st.multiselect(
            "🏷️ Nhóm Dịch Vụ",
            df_all["Nhóm dịch vụ"].unique(),
            default=df_all["Nhóm dịch vụ"].unique(),
        )
    with f_col4:
        day_range = st.slider(
            "📅 Ngày trong tháng", 
...
df_filtered = df_all[
    ...
    & (df_all["Day"] >= day_range[0])
    & (df_all["Day"] <= day_range[1])
]
...
st.subheader(f"1️⃣ TỔNG QUAN CHỈ SỐ (So sánh từ ngày {day_range[0]} đến {day_range[1]})")
...
if cust_t7 > cust_t6 and aov_t7 < aov_t6:
    ...
    insight_text += (
        "1. **Tăng trưởng bền vững:** Doanh thu và AOV đều tăng trưởng ấn tượng.\n"
    )
...
with c2:
    cat_rev = (
        df_filtered.groupby(["Tháng", "Danh mục"])["Doanh thu"].sum().reset_index()
    )
    fig3 = px.bar(
        cat_rev,
        x="Danh mục",
...
with c3:
    mix_rev = (
        df_filtered.groupby(["Tháng", "Nhóm dịch vụ"])["Doanh thu"].sum().reset_index()
    )
    fig2 = px.bar(
        mix_rev,
        x="Nhóm dịch vụ",
...
with c4:
    ...
    with p1:
        fig_pie_t6 = px.pie(
            ...
            names="Danh mục",
            color="Danh mục",
            title="Tỷ Trọng (Tháng Trước)",
            hole=0.4,
            color_discrete_map=cat_colors,
        )
        fig_pie_t6.update_traces(
            textposition="inside",
            textinfo="percent",
            marker=dict(line=dict(color="#FFFFFF", width=1)),
        )
        fig_pie_t6.update_layout(
            font_family="Lexend",
            showlegend=True,
            legend=dict(
                orientation="h", yanchor="top", y=-0.2, xanchor="center", x=0.5
            ),
            margin=dict(t=30, b=120, l=0, r=0),
        )
    ...
    with p2:
        fig_pie_t7 = px.pie(
            ...
            names="Danh mục",
            color="Danh mục",
            title="Tỷ Trọng (Tháng Hiện Tại)",
            hole=0.4,
            color_discrete_map=cat_colors,
        )
        fig_pie_t7.update_traces(
            textposition="inside",
            textinfo="percent",
            marker=dict(line=dict(color="#FFFFFF", width=1)),
        )
        fig_pie_t7.update_layout(
            font_family="Lexend",
            showlegend=True,
            legend=dict(
                orientation="h", yanchor="top", y=-0.2, xanchor="center", x=0.5
...
st.dataframe(
    df_filtered.sort_values(by="Ngày", ascending=False).astype(str),
